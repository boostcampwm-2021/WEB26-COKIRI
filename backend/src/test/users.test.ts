...
describe('회원가입', () => {
  ...
  test('유저네임 유효성 검사', async () => {
    const responseTrue = await request(app).get('/v1/users?query=test1');
    expect(responseTrue.body.data).toBe(false);
    expect(responseTrue.status).toBe(200);

    const responseFalse = await request(app).get('/v1/users?query=test1');
