export const DEFAULT_PROFILE_IMAGE = '/images/default_profile_image.jpg';
export const LOGO_IMAGE = '/images/logo_image.svg';
export const POST_IMAGE_WIDTH = 600;
export const POST_IMAGE_HEIGHT = 300;
export const LOGO_IMAGE_WIDTH = 64;
export const LOGO_IMAGE_HEIGHT = 24;
export const DEFAULT_BUTTON_WIDTH = 48;
export const DEFAULT_BUTTON_HEIGHT = 48;
export const DEFAULT_BUTTON_MARGIN = 8;
export const FLOATING_BUTTON_WIDTH = 56;
export const FLOATING_BUTTON_HEIGHT = 56;
export const FLOATING_BUTTON_MARGIN = 0;
export const LOGO_BUTTON_WIDTH = 144;
export const LOGO_BUTTON_HEIGHT = 48;
export const FOLLOW_BUTTON_WIDTH = 64;
export const FOLLOW_BUTTON_HEIGHT = 32;
export const FOLLOW_BUTTON_MARGIN = 8;
export const SOCIAL_BUTTON_WIDTH = 64;
export const SOCIAL_BUTTON_HEIGHT = 64;
export const SOCIAL_BUTTON_MARGIN = 16;
export const HEADER_ICON_SIZE = `24`;
export const SOCIAL_ICON_SIZE = `32`;
export const POST_CARD_WIDTH = 600;
export const POST_CARD_HEIGHT = 0;
